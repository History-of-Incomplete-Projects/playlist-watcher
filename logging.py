...
class Logging(object):
    def __init__(self, logbook_name):
        self.logbook_name = logbook_name
        self.logbook = {"playlists": []}
        ...
        for logged_playlist in self.logbook["playlists"]:
            if logged_playlist["url"] == playlist.url:
                logged_playlist["downloaded_videos"].append(
                    {"url": video.url, "artist": video.artist, "title": video.title}
                )
    ...
    def write_to_logbook(self):
        with open(self.logbook_name, "w+", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    self.logbook, f, ensure_ascii=False, indent=4, sort_keys=True
                )
            )

    def read_from_logbook(self):
        try:
            with open(self.logbook_name, encoding="utf-8") as f:
